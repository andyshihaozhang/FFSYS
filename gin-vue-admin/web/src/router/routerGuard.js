...
function addTopLevelIfAbsent(router,r) {
...
function addRouteByChildren(router, route, segments = [], parentName = null) {
  // 跳过外链根节点
  ...
  if (route?.name === 'layout') {
    route.children?.forEach((child) => addRouteByChildren(router, child, [], null))
    return
  }
  ...
  if (route?.meta?.defaultMenu === true && parentName === null) {
    ...
    addTopLevelIfAbsent(router, newRoute)

    // 若该 defaultMenu 节点仍有子节点，继续递归处理其子节点（挂载到该顶级路由下）
    if (children.length) {
      // 重置片段，使其成为顶级下的二级相对路径
      children.forEach((child) => addRouteByChildren(router, child, [], newRoute.name))
    }
    return
  }
  ...
  if (route?.children && route.children.length) {
    ...
    route.children.forEach((child) => addRouteByChildren(router, child, nextSegments, parentName))
    return
  }
...
// 处理路由加载
const setupRouter = async (router, userStore) => {
  try {
  ...
  toRegister.forEach((r) => addRouteByChildren(router, r, [], null))
    return true
  } catch (error) {
...
// 处理路由重定向
const handleRedirect = (router, to, userStore) => {
...
export const routerReGuardInit = (router) => {
  // 路由守卫
  router.beforeEach(async (to, from) => {
    ...
    if (WHITE_LIST.includes(to.name)) {
      if (token) {
        if(!routerStore.asyncRouterFlag){
          await setupRouter(router, userStore)
        }
    ...
    if (token) {
      ...
      if (!routerStore.asyncRouterFlag && !WHITE_LIST.includes(from.name)) {
        const setupSuccess = await setupRouter(router, userStore)

        if (setupSuccess && userStore.token) {
          return handleRedirect(router, to, userStore)
        }
